...
import {updateRecurringEvent,getEventType,getSingleRecurringEvent } from "service/api";
...
function EditRecurringEvent({ isOpen, onRequestClose, eventId }) {
...
const cid= localStorage.getItem("LoginId");
  ...
  useEffect(() => {
    async function loadEvent() {
      setIsLoading(true)
      try {
        const res = await getSingleRecurringEvent({ data: { id: eventId } });
        console.log('Fetched Data For Single Event:', res.data);
        ...
        if (res.data) {
          ...
          let daysString = res.data.event_day;
            const daysArray = daysString.split(',');

            setEventDay(daysArray)
 ...
 const handleCheckboxChange = (changedDay) => {
  const lowercaseChangedDay = changedDay;

  // Check if the lowercase day is already in the array
  if (eventDay.includes(lowercaseChangedDay)) {
    // If it is, remove it
    const updatedEventDay = eventDay.filter((day) => day !== lowercaseChangedDay);
    setEventDay(updatedEventDay);
  } else {
    // If it is not, add it
    const updatedEventDay = [...eventDay, lowercaseChangedDay];
    setEventDay(updatedEventDay);
  }
};
  ...
  async function handleEdit() {
    ...
    try {
      const res = await updateRecurringEvent(req);
      console.log(res);
...
console.log(selectedEventType,"selected event type ",eventId ,"=====>>>>>>>>>>>>>>> this ===>>")
  ...
  console.log(eventDay,"getting response from api ===>>")
  return (
    <Modal
    isOpen={isOpen}
    ...
    {weekdays.map((weekday) => (
      <li key={weekday} className="items-center space-x-2 mr-6">
        <input
          ...
          checked={eventDay.includes(weekday)}
